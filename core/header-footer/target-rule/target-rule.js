;(function ( $, window, undefined ) {
			...
			ajax: {
			    ...
			    data: function (params) {
			      	return {
				        ...
				        page: params.page,
						action: 'aiea_hf_get_posts_by_query',
	...
	var update_target_rule_input = function(wrapper) {
		...
		wrapper.find('.ai-target-rule-condition').each(function(i) {
	...
	var update_close_button = function(wrapper) {
		...
		rules 		= wrapper.find('.ai-target-rule-condition');
		show_close	= false;
	...
	var update_exclusion_button = function( force_show, force_hide ) {
		...
		var exclude_conditions = exclude_on.find('.ai-target-rule-condition');
	...
	var aiea_hide_shortcode_field = function() {
		var selected = $('#aiea_hf_template_type').val() || 'none';
		$( '.ai-hf-options-table' ).removeClass().addClass( 'ai-hf-options-table widefat ai-hf-selected-template-type-' + selected );
	}
	...
	$(document).ready(function($) {
		...
		$("#postbox-container-1 > div > .postbox:not(#submitdiv)").remove();
		$("#postbox-container-2 > div > .postbox:not(#ai-hf-meta-box)").remove();
		
		$document.on( 'change', '#aiea_hf_template_type', function( e ) {
			aiea_hide_shortcode_field();
		});
		aiea_hide_shortcode_field();

		jQuery( '.ai-target-rule-condition' ).each( function() {
			var $this 			= $( this ),
		...
		jQuery( document ).on( 'change', '.ai-target-rule-condition select.target_rule-condition' , function( e ) {
			...
			if( 'specifics' == this_val ) {
				$this.closest( '.ai-target-rule-condition' ).next( '.target_rule-specific-page-wrap' ).slideDown( 300 );
			} else {
				$this.closest( '.ai-target-rule-condition' ).next( '.target_rule-specific-page-wrap' ).slideUp( 300 );
			}
		...
		jQuery( '.ast-target-rule-selector-wrapper' ).on( 'click', '.target_rule-add-rule-wrap a', function(e) {
			...
			var $this 	= jQuery( this ),
				...
				template  = wp.template( 'ai-target-rule-' + type + '-condition' ),
				field_wrap 		= $this.closest('.ast-target-rule-wrapper');
		...
		jQuery( '.ast-target-rule-selector-wrapper' ).on( 'click', '.target_rule-condition-delete', function(e) {
			var $this 			= jQuery( this ),
				rule_condition 	= $this.closest('.ai-target-rule-condition'),
				field_wrap 		= $this.closest('.ast-target-rule-wrapper');
				cnt 			= 0,
			...
			if ( 'exclude' == data_type ) {
				...
				if ( 1 === field_wrap.find('.target_rule-condition').length ) {
					...
					$this.parent('.ai-target-rule-condition').next('.target_rule-specific-page-wrap').remove();
			...
			field_wrap.find('.ai-target-rule-condition').each(function(i) {
