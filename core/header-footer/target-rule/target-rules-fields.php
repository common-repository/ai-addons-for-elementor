 ...
 * Meta Boxes setup
 */
class AIEA_Target_Rules_Fields {
	...
	public function __construct() {
		...
		define( 'AIEA_TARGET_RULE_VER', '1.0.0' );
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'aiea_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page', 'ai-addons' ),
			'special-search' => __( 'Search Page', 'ai-addons' ),
			'special-blog'   => __( 'Blog / Posts Page', 'ai-addons' ),
			'special-front'  => __( 'Front Page', 'ai-addons' ),
			'special-date'   => __( 'Date Archive', 'ai-addons' ),
			'special-author' => __( 'Author Archive', 'ai-addons' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'ai-addons' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'ai-addons' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'ai-addons' ),
					'basic-singulars' => __( 'All Singulars', 'ai-addons' ),
					'basic-archives'  => __( 'All Archives', 'ai-addons' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'ai-addons' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'ai-addons' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'ai-addons' ),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'aiea_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'ai-addons' ),
				'value' => array(
					'all'        => __( 'All', 'ai-addons' ),
					'logged-in'  => __( 'Logged In', 'ai-addons' ),
					'logged-out' => __( 'Logged Out', 'ai-addons' ),
				),
			),

			'ai' => array(
				'label' => __( 'AI', 'ai-addons' ),
				'value' => array(),
			),
		);
		...
		foreach ( $roles as $slug => $data ) {
			$selection_options['ai']['value'][ $slug ] = $data['name'];
		}
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'aiea_user_roles_list', $selection_options );
	}
	...
	public static function get_user_by_key( $key ) {
		...
		} elseif ( $user_selection['ai']['value'][ $key ] ) {
			return $user_selection['ai']['value'][ $key ];
		}
		return $key;
	}	
	...
	public function admin_styles() {
		...
		wp_enqueue_style( 'ai-hf-admin', AIEA_URL . 'core/header-footer/assets/css/hf-admin.css', false, '1.0.0' );
		
		wp_enqueue_script( 'ai-select2', AIEA_URL . 'core/header-footer/target-rule/select2.js', array( 'jquery' ), AIEA_TARGET_RULE_VER, true );
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( AIEA_DIR . 'core/header-footer/target-rule/i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
				$ast_lang = $select2_lang[ $wp_lang ];
				wp_enqueue_script(
					'ai-select2-lang',
					AIEA_URL . 'core/header-footer/target-rule/i18n/' . $select2_lang[ $wp_lang ] . '.js',
					array(
						'jquery',
						'ai-select2',
					),
					AIEA_TARGET_RULE_VER,
					true
				);
			}
		}
		...
		wp_register_script(
			'ai-target-rule',
			AIEA_URL . 'core/header-footer/target-rule/target-rule.js',
			array(
				'jquery',
				'ai-select2',
			),
		...
		wp_enqueue_script( 'ai-target-rule' );

		wp_register_script(
			'ai-user-role',
			AIEA_URL . 'core/header-footer/target-rule/user-role.js',
			array(
				'jquery',
			),
			AIEA_TARGET_RULE_VER,
		...
		wp_enqueue_script( 'ai-user-role' );

		wp_register_style( 'ai-select2', AIEA_URL . 'core/header-footer/target-rule/select2.css', '', AIEA_TARGET_RULE_VER );
		wp_enqueue_style( 'ai-select2' );
		wp_register_style( 'ai-target-rule', AIEA_URL . 'core/header-footer/target-rule/target-rule.css', '', AIEA_TARGET_RULE_VER );
		wp_enqueue_style( 'ai-target-rule' );
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'ai-addons' ),
			'please_delete' => __( 'Please delete', 'ai-addons' ),
			'more_char'     => __( 'or more characters', 'ai-addons' ),
			'character'     => __( 'character', 'ai-addons' ),
			'loading'       => __( 'Loading more results…', 'ai-addons' ),
			'only_select'   => __( 'You can only select', 'ai-addons' ),
			'item'          => __( 'item', 'ai-addons' ),
			'char_s'        => __( 's', 'ai-addons' ),
			'no_result'     => __( 'No results found', 'ai-addons' ),
			'searching'     => __( 'Searching…', 'ai-addons' ),
			'not_loader'    => __( 'The results could not be loaded.', 'ai-addons' ),
			'search'        => __( 'Search pages / post / categories', 'ai-addons' ),
			'ajax_nonce'    => wp_create_nonce( 'ai-hf-get-posts-by-query' ),
		);
		wp_localize_script( 'ai-select2', 'astRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'ai-addons' );
		$saved_values   = $value;
		...
		/* WP Template Format */
		echo '<script type="text/html" id="tmpl-ai-target-rule-' . esc_attr( $rule_type ) . '-condition">';
		echo '<div class="ai-target-rule-condition ast-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
		echo '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
		/* Condition Selection */
		echo '<div class="target_rule-condition-wrap" >';
		echo '<select name="' . esc_attr( $input_name ) . '[rule][{{data.id}}]" class="target_rule-condition form-control ast-input">';
		echo '<option value="">' . __( 'Select', 'ai-addons' ) . '</option>';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			foreach ( $group_data['value'] as $opt_key => $opt_value ) {
				echo '<option value="' . esc_attr( $opt_key ) . '">' . esc_html( $opt_value ) . '</option>';
			}
			echo '</optgroup>';
		}
		echo '</select>';
		...
		echo '</div> <!-- ai-target-rule-condition -->';

		/* Specific page selection */
		echo '<div class="target_rule-specific-page-wrap" style="display:none">';
		echo '<select name="' . esc_attr( $input_name ) . '[specific][]" class="target-rule-select2 target_rule-specific-page form-control ast-input " multiple="multiple">';
		echo '</select>';
		echo '</div>';
		...
		/* Wrapper Start */
		echo '<div class="ast-target-rule-wrapper ast-target-rule-' . esc_attr( $rule_type ) . '-on-wrap" data-type="' . esc_attr( $rule_type ) . '">';
		echo '<div class="ast-target-rule-selector-wrapper ast-target-rule-' . esc_attr( $rule_type ) . '-on">';
		echo self::generate_target_rule_selector( $rule_type, $selection_options, $input_name, $saved_values, $add_rule_label );
		echo '</div>';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'ai-addons' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'ai-addons' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'ai-addons' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="ai-target-rule-condition ast-target-rule-' . esc_attr( $index ) . '" data-rule="' . esc_attr( $index ) . '" >';
			/* Condition Selection */
			...
			$output .= '<select name="' . esc_attr( $input_name ) . '[rule][' . esc_attr( $index ) . ']" class="target_rule-condition form-control ast-input">';
			$output .= '<option value="">' . __( 'Select', 'ai-addons' ) . '</option>';

			foreach ( $selection_options as $group => $group_data ) {
				$output .= '<optgroup label="' . esc_attr( $group_data['label'] ) . '">';
				foreach ( $group_data['value'] as $opt_key => $opt_value ) {
					$output .= '<option value="' . esc_attr( $opt_key ) . '" ' . selected( $data, $opt_key, false ) . '>' . esc_html( $opt_value ) . '</option>';
				}
				$output .= '</optgroup>';
			}
			$output .= '</select>';
			...
			if ( 'specifics' == $data && isset( $saved_values['specific'] ) && null != $saved_values['specific'] && is_array( $saved_values['specific'] ) ) {
				foreach ( $saved_values['specific'] as $data_key => $sel_value ) {
					...
					if ( strpos( $sel_value, 'post-' ) !== false ) {
						...
						$output    .= '<option value="post-' . esc_attr( $post_id ) . '" selected="selected" >' . esc_html( $post_title ) . '</option>';
					}
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'ai-addons' ) . '</a>';
			$output .= '</div>';
		}
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'aiea_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		echo '<script type="text/html" id="tmpl-ai-user-role-condition">';
			echo '<div class="ai-user-role-condition ast-user-role-{{data.id}}" data-rule="{{data.id}}" >';
				echo '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				echo '<div class="user_role-condition-wrap" >';
					echo '<select name="' . esc_attr( $input_name ) . '[{{data.id}}]" class="user_role-condition form-control ast-input">';
					echo '<option value="">' . __( 'Select', 'ai-addons' ) . '</option>';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			echo '</optgroup>';
					...
					echo '</select>';
				echo '</div>';
			echo '</div> <!-- ai-user-role-condition -->';
		echo '</script>';
		...
		echo '<div class="ast-user-role-wrapper ast-user-role-display-on-wrap" data-type="display">';
			echo '<div class="ast-user-role-selector-wrapper ast-user-role-display-on">';
				echo '<div class="user_role-builder-wrap">';
		foreach ( $saved_values as $index => $data ) {
			echo '<div class="ai-user-role-condition ast-user-role-' . esc_attr( $index ) . '" data-rule="' . esc_attr( $index ) . '" >';
				echo '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				echo '<div class="user_role-condition-wrap" >';
					echo '<select name="' . esc_attr( $input_name ) . '[' . esc_attr( $index ) . ']" class="user_role-condition form-control ast-input">';
					echo '<option value="">' . __( 'Select', 'ai-addons' ) . '</option>';

			foreach ( $selection_options as $group => $group_data ) {
				echo '<optgroup label="' . esc_attr( $group_data['label'] ) . '">';
				foreach ( $group_data['value'] as $opt_key => $opt_value ) {
					echo '<option value="' . esc_attr( $opt_key ) . '" ' . selected( $data, $opt_key, false ) . '>' . esc_html( $opt_value ) . '</option>';
				}
				echo '</optgroup>';
			}
					echo '</select>';
				...
				echo '</div>';
				/* Add new rule */
				echo '<div class="user_role-add-rule-wrap">';
				echo '<a href="#" class="button" data-rule-id="' . absint( $index ) . '">' . $add_rule_label . '</a>';
				echo '</div>';
			echo '</div>';
		echo '</div>';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'aiea_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
		...
		if ( ! empty( $already_set_rule ) ) {
			add_action(
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf( __( 'The same display setting is already exist in %s post/s.', 'ai-addons' ), $rule_set_titles );
 ...
 */
AIEA_Target_Rules_Fields::get_instance();
