...
/**
 * AI Addons Archive Title
 *
 * @since 1.0.0
 */
 
class AIEA_Elementor_Archive_Title_Widget extends Widget_Base {
	
	/**
	 * Get widget name.
	 *
	 * Retrieve Archive Title name.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_name() {
		return 'ai-archive-title';
	}

	/**
	 * Get widget title.
	 *
	 * Retrieve Archive Title title.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_title() {
		return __( 'Archive Title ', 'ai-addons' );
	}

	/**
	 * Get widget icon.
	 *
	 * Retrieve Archive Title icon.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_icon() {
		return 'ai-default-icon '. aiea_addon_base()->widget_icon_classes('archive-title');
	}
	 ...
	 *
	 * Retrieve the list of categories the Archive Title widget belongs to.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_keywords() {
		return [ 'title', 'archive title', 'wp title', 'blog title' ];
	}
			
	public function get_help_url() {
        return 'https://aiaddons.ai/ai-archive-title/';
    }

	/**
	 * Register Archive Title widget controls. 
	 *
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'aiea_section',
			[
				'label'	=> esc_html__( 'Archive Title ', 'ai-addons' ),
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ai-archive-title' => 'text-transform: {{VALUE}};'
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ai-archive-title' => 'text-align: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' 		=> '',
				'selectors' => [
					'{{WRAPPER}} .ai-archive-title' => 'color: {{VALUE}};'
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' 		=> '',
				'selectors' => [
					'{{WRAPPER}} .ai-archive-title:hover' => 'color: {{VALUE}};'
				],
			]
		);		
		$this->end_controls_tab();
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ai-archive-title' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				]
			]
		);		
		$this->add_responsive_control(
			...
			[
				...
				'type' => Controls_Manager::DIMENSIONS,
				'selectors' => [
					'{{WRAPPER}} .ai-archive-title' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				]
			]
		);
		$this->add_group_control(
			...
			[
				...
				'selector' 		=> '{{WRAPPER}} .ai-archive-title'
			]
		);
		$this->add_control(
			'text_shadow_opt',
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{SELECTOR}} .ai-archive-title' => 'text-shadow: {{HORIZONTAL}}px {{VERTICAL}}px {{BLUR}}px {{COLOR}};',
				],
			]
		);
		$this->end_controls_section();
	...
	/**
	 * Render Archive Title widget output on the frontend.
	 *
	...
	protected function render() {
		...
		echo '<div class="ai-archive-title-wrapper">';			
				echo '<'. esc_attr( $title_tag ) .' class="ai-archive-title">'. get_the_archive_title() .'</'. esc_attr( $title_tag ) .'><!-- .ai-archive-title -->';
