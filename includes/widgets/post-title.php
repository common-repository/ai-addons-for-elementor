...
/**
 * AI Addons Post Title 
 *
 * @since 1.0.0
 */
 
class AIEA_Elementor_Post_Title_Widget extends Widget_Base {
	
	/**
	 * Get widget name.
	 *
	 * Retrieve Post Title name.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_name() {
		return 'ai-post-title';
	}

	/**
	 * Get widget title.
	 *
	 * Retrieve Post Title title.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_title() {
		return __( 'Post Title', 'ai-addons' );
	}

	/**
	 * Get widget icon.
	 *
	 * Retrieve Post Title icon.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_icon() {
		return 'ai-default-icon '. aiea_addon_base()->widget_icon_classes('post-title');
	}
	...
	public function get_keywords() {
		return [ 'title', 'post title', 'wp title', 'blog title' ];
	}
			
	public function get_help_url() {
        return 'https://aiaddons.ai/ai-post-title/';
    }
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'aiea_section',
			[
				'label'	=> esc_html__( 'Post Title', 'ai-addons' ),
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ai-post-title' => 'text-transform: {{VALUE}};'
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ai-post-title' => 'text-align: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' 		=> '',
				'selectors' => [
					'{{WRAPPER}} .ai-post-title' => 'color: {{VALUE}};'
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' 		=> '',
				'selectors' => [
					'{{WRAPPER}} .ai-post-title:hover' => 'color: {{VALUE}};'
				],
			]
		);		
		$this->end_controls_tab();
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ai-post-title' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				]
			]
		);		
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::DIMENSIONS,
				'selectors' => [
					'{{WRAPPER}} .ai-post-title' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				]
			]
		);
		$this->add_group_control(
			...
			[
				...
				'selector' 		=> '{{WRAPPER}} .ai-post-title'
			]
		);
		$this->add_control(
			'text_shadow_opt',
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{SELECTOR}} .ai-post-title' => 'text-shadow: {{HORIZONTAL}}px {{VERTICAL}}px {{BLUR}}px {{COLOR}};',
				],
			]
		);
		$this->end_controls_section();
	...
	protected function render() {
		...
		echo '<div class="ai-post-title-wrapper">';			
				echo '<'. esc_attr( $title_tag ) .' class="ai-post-title">'. get_the_title() .'</'. esc_attr( $title_tag ) .'><!-- .ai-post-title -->';
