	...
	$.fn.extend({
		...
		makeAITable: function ( options ){
			...
			if( !sort_opt && search_opt ){
				...
				//Pagination Element			
				if( pagination_opt ){
					$(aiea_table).makeAIPagination({
						parent: settings.pagination_ele,
						max_row: settings.pagination_max_row					
					});				
				} else {
					$(aiea_table).find("tr").css({ 'display' : '' });
				}
			}
        ...
        makeAISortable: function( options ){
            ...
            $(aiea_table).find('th').click(function(){
		...
		makeAISearchable: function( options  ){		
			var settings = $.extend({
				value: '',
				pagination: '',
				pagination_ele: '',
			...
			if( pagination ){
				if( value == '' ){
					...
					$(pagination_ele).show();
					setTimeout( function(){ $(aiea_table).makeAIPagination({ max_row: max_row }); }, 10 );
				}else{
					var pagination_ele = settings.pagination_ele;
					$(pagination_ele).hide();
				}
			...
			aiea_table.find('tr').filter( function() {
				$(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
			});			
		},
		makeAIPagination: function( options  ){	
			var settings = $.extend({
				max_row: 10,
				parent: ''
			}, options );
			...
			var tr = $(aiea_table).find("tbody tr");
			...
			$(tr).each(function(i){
				...
				$(pagi_parent).find('a').click(function(e){
					
					$(this).parent(".ai-data-table-pagination-wrap").find("a").removeClass("active");
					$(this).addClass("active");
					
					e.preventDefault();
					$tr = $(aiea_table).find("tbody tr");
					$tr.hide();
					var page = $(this).text();
					var temp = page - 1;
